		...
		function setColorPalette($color_set, $color_rand, $colors) {
			...
			if (! empty ( $colors )) {
				...
				if ($color_set == 'random') {
					...
					MAXICHARTSAPI::getLogger ()->error ( "Unkown color set: " . $color_set );
					$color_set = 'default';
				}
      ...
      ticks: {
		...
		".$stepSizeX."
                ...
                if ($chartType === 'halfDoughnut' || $chartType === 'halfPie'){
                    ...
                    $currentchartOptions .= ',';
                    $currentchartOptions .= 'rotation: 1 * Math.PI,';
                    $currentchartOptions .= 'circumference: 1 * Math.PI,';
                }
                ...
                if (!empty($datasets_field)){
                    MAXICHARTSAPI::getLogger()->debug("Datasets field chosen : ".$datasets_field);
                    MAXICHARTSAPI::getLogger()->debug($chartToPlot);
                    $chartToPlot['labels'] = array_map(function($in) { $maxSize = 30;$out = strlen($in) > $maxSize ? substr($in,0,$maxSize)."..." : $in; return $out; }, $chartToPlot['labels']);
                    //$currentchartOptions .= "scale:{scaleOverride:true,scaleSteps:10,scaleStepWidth:1,scaleStartValue:0},";
                    
                    //$currentchartOptions .= 'scaleOverride:true,scaleSteps:10,scaleStepWidth:1,scaleStartValue:0,';
                    
                    //$currentchartData .= implode('","', $labelsOut);
                    //$currentchartData .= 'labels: "' . $chartToPlot['labels'] . '",';
                }
                    ...
                    case 'radar':
                        ...
                        if (isset($chartToPlot['labels']) && is_array($chartToPlot['labels'])) {
                            ...
                            if (isset($chartToPlot['data'])){
                            	...
                            	MAXICHARTSAPI::getLogger()->error('No data to build labels');
                            	MAXICHARTSAPI::getLogger()->error($chartToPlot);
                            	$currentchartData .= implode('","', $labelsCleaned);
                            }
                        ...
                        if ($chartToPlot['type'] === 'csv' && count($chartToPlot['datasets']) <= 1) {
                            $currentDataset = '{';
                            $currentDataset .= '
                            ...
                            $currentchartDatasets .= $currentDataset;
                        		...
                        		!is_numeric($atts['list_field_id']) && isset($multiRows) && $multiRows
                        		||
                        		isset($chartToPlot['multisets']) && $chartToPlot['multisets']
                        		) {
